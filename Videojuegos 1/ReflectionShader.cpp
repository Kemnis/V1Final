...
ReflectionShader::ReflectionShader(std::string vsSource, std::string psSource) : Shader()
{
	//m_vertexShader = 0;
	//m_pixelShader = 0;
	//m_layout = 0;
	...
	//m_clipPlaneBuffer = 0;
	//m_lightBuffer = 0;
	this->type = ShaderType::ReflectionShader;
	ConstantBuffer matrixConstantBuffer(MATRIX_BUFFER_ID, ConstantBufferLocation::VertexShader, sizeof(ConstantBufferTypes::MatrixBufferDefault));
...
bool ReflectionShader::Initialize(std::string vsSource, std::string psSource)
{
	bool result;
	...
	ConstantBuffer ClipPlaneBuffer(CUSTOM_BUFFER_1, ConstantBufferLocation::VertexShader, sizeof(ConstantBufferTypes::ClipPlaneBufferType));
	this->AddConstantBuffer("ClipPlaneBuffer", ClipPlaneBuffer);

	ConstantBuffer LightBuffer(CUSTOM_BUFFER_2, ConstantBufferLocation::PixelShader, sizeof(ConstantBufferTypes::LightBufferType));
	this->AddConstantBuffer("LightBuffer", LightBuffer);
...
//	//
//	result = D3DCompileFromFile(vsFilename, NULL, NULL, "ReflectionVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &vertexShaderBuffer,
//		&errorMessage);
//	if(FAILED(result))
//	{
...
//	result = D3DCompileFromFile(psFilename, NULL, NULL, "ReflectionPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &pixelShaderBuffer,
//		&errorMessage);
//
//	if(FAILED(result))
//	{
