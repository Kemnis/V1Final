...
bool Water::Initialize(ID3D11Device* device, ID3D11DeviceContext*deviceContext, ID3D11Buffer *VertexBuffer, ID3D11Buffer*IndexBuffer, float waterHeight, float waterRadius)
{
	bool result;
	...
	result = InitializeBuffers(device, VertexBuffer, IndexBuffer, waterRadius);
	if(!result)
	{
		return false;
	}
...
bool Water::InitializeBuffers(ID3D11Device* device, ID3D11Buffer *VertexBuffer, ID3D11Buffer*IndexBuffer, float waterRadius)
{
	VertexType* vertices;
