...
WaterShader::WaterShader(std::string vsSource, std::string psSource) : Shader()
{
	//m_vertexShader = 0;
	//m_pixelShader = 0;
	//m_layout = 0;
	...
	//m_camNormBuffer = 0;
	//m_waterBuffer = 0;
	this->type = ShaderType::WaterShader;
	ConstantBuffer matrixConstantBuffer(MATRIX_BUFFER_ID, ConstantBufferLocation::VertexShader, sizeof(ConstantBufferTypes::MatrixBufferReflection));
...
bool WaterShader::Initialize(std::string vsSource, std::string psSource)
{
	bool result;
	...
	ConstantBuffer CamnNormConstantBuffer(CUSTOM_BUFFER_1, ConstantBufferLocation::VertexShader, sizeof(ConstantBufferTypes::CamNormBufferType));
	this->AddConstantBuffer("CamNormBuffer", CamnNormConstantBuffer);


	ConstantBuffer WaterBufferConstantBuffer(CUSTOM_BUFFER_2, ConstantBufferLocation::PixelShader, sizeof(ConstantBufferTypes::WaterBufferType));
	this->AddConstantBuffer("WaterBuffer", WaterBufferConstantBuffer);
...
bool WaterShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = D3DCompileFromFile(vsFilename, NULL, NULL, "WaterVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &vertexShaderBuffer,
		&errorMessage);
	...
	result = D3DCompileFromFile(psFilename, NULL, NULL, "WaterPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &pixelShaderBuffer,
		&errorMessage);
	if(FAILED(result))
	{
		// If the shader failed to compile it should have writen something to the error message.
